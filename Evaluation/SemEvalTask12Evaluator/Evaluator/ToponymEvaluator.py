...
class Toponym(object):
    ...
    def __init__(self, ID, startOffset, endOffset, text):
        ...
        assert self.start <= self.end, "The start offset {} < the end offset {}, which should not be.".format(self.start, self.end)
        self.text = text
        self.geoID = None
        self.long = None
        self.lat  = None
    ...
    def setGeoID(self, geoID):
        if geoID=="-1":
            geoID="NA"
        if geoID != "NA":
            assert geoID.isdigit(), "A geoID [{}] is incorrectly formated, should be all digits, check the data.".format(geoID)
        self.geoID = geoID
    def getGeoID(self):
        return self.geoID
    ...
    def __parseExcludedSpan__(self, lines, filePath):
        ...
        for line in lines:
            if re.match("T[0-9]+\t.*", line):
                ...
                assert len(components)==3, "Unexpected format in the file {} for the line {}, was expecting 3 substrings tab separated.".format(filePath, line)
                if re.match(r'Protein \d+ \d+', components[1]):
                    ...
                    annotations[components[0][1:]] = Annotation(components[0][1:], elements[0], offsets[0], offsets[-1], components[2])
            index = index+1
...
#                     annotation.setComment(components[1], components[2])
                assert components[1].startswith("AnnotatorNotes T"), "Unexpected format in the file {} for the line {}, was expecting AnnotatorNotes as second element of the comment".format(filePath, line)
                if components[1][16:] in annotations:# else is possible if we have a comment for the Location or other annotations
                    annotation = annotations[components[1][16:]]
                    assert annotation.getComment() is None, "Found a Excluded Span annotation that has more than one comment, in the file {} for the line {}".format(filePath, line)
                    annotation.setComment(components[1], components[2])
        #check that all ExcludedSpan annotations have found their comments
        for annotation in annotations.values():
            assert annotation.getComment() is not None, "Found an Excluded Span annotation that does not have a comment: {}".format(annotation.getID())
        
        #Now we create the list of ExcludedSpan objects
        spanEndOffsets = {}
        for annotation in annotations.values():
            assert annotation.getEndOffset() not in spanEndOffsets, "Found 2 excluded spans ending at the same position: {}, which should not, in file {}".format(annotation.getEndOffset(), filePath)
            spanEndOffsets[int(annotation.getEndOffset())] = annotation
        endOffsets = sorted(spanEndOffsets.keys())
        assert len(endOffsets)%2 == 0, "Found an odd number of excluded spans' offsets: {}, one is probably missing, in file {}".format(len(spanEndOffsets), filePath)
        ...
        while index < len(endOffsets):
            ...
            assert beginSpanAnn.getComment()[1]=="BEGIN" and endSpanAnn.getComment()[1]=="END", "Found 2 successive offsets ({} and {}) that do not refer to a beginning and end of a Excluded span, in file {}".format(endOffsets[index], endOffsets[index+1], filePath)
            exSpan = ExcludedSpan()            
            exSpan.setBegin(beginSpanAnn.getID(), beginSpanAnn.getStartOffset(), beginSpanAnn.getEndOffset(), beginSpanAnn.getText())
            exSpan.setEnd(endSpanAnn.getID(), endSpanAnn.getStartOffset(), endSpanAnn.getEndOffset(), endSpanAnn.getText())
...
#                     annotation.setComment(components[1], components[2])
        ...
        for annotation in annotations.values():
            assert annotation.getComment() is not None, "Found an Location annotation that does not have a comment: {}".format(annotation.getID())
        ...
        toponyms = []
        for annotation in annotations.values():
            topo = Toponym(annotation.getID(), annotation.getStartOffset(), annotation.getEndOffset(), annotation.getText())
            ...
            if m:
                ...
                assert len(latlong)==2, "Found a longitude and latitude incorrectly formatted {}, in in the file {}".format(annotation.getComment()[1], filePath)
    ...
    def __getTextContainingTabs__(self, line, components):
        ...
        ['T33', 'Location 6173 6182', 'Sri\tLanka']
        """
        assert(re.match("T[0-9]+",components[0])), "Unexpected format of the line, impossible to find the brat ID in: {}".format(line)
        assert(re.match("Location \d+ \d+", components[1])), "Unexpected format of the line, impossible to find the Location in: {}".format(line)
...
class Prediction(object):
    ...
    def __init__(self, value, toponymPredicted, toponymReference):
        ...
        """
        self.value = value
        assert self.value == 'TP' or self.value == 'FP' or self.value == 'FN', "I found a value for the prediction different from what I expect ('TP', 'FP', 'FN'): {}, check the code.".format(value)
...
class Answer(object):
    ...
    def addTcd(self, Tcd):
        ...
        if Tcd.topoPred is not None:
            assert Tcd.topoPred.getGeoID() is not None, "Found a toponym in a prediction {} which does not have a geoID or 'NA' value in the disambiguation or Resolution, check the data.".format(str(Tcd))
        if Tcd.topoRef is not None:
            assert Tcd.topoRef.getGeoID() is not None, "Found a toponym in a prediction {} which does not have a geoID or 'NA' value in the disambiguation or Resolution, check the data.".format(str(Tcd)) 
    ...
    def addTid(self, Tid):
        ...
        if Tid.topoPred is not None:
            assert Tid.topoPred.getGeoID() is not None, "Found a toponym in a prediction {} which does not have a geoID or 'NA' value in the disambiguation or Resolution, check the data.".format(str(Tid))
        if Tid.topoRef is not None:
            assert Tid.topoRef.getGeoID() is not None, "Found a toponym in a prediction {} which does not have a geoID or 'NA' value in the disambiguation or Resolution, check the data.".format(str(Tid))
...
class Oracle(object):
    ...
    def __addAnnotationsToMap__(self, annotations):
        ...
        for ann in annotations:
            ...
            index = startIndex
            assert index <= ann.getEndOffset(), "Found a {} which offsets are incoherent start: {} > end: {}".format(type(ann), ann.getStartOffset(), ann.getEndOffset())
            while index <= ann.getEndOffset():
                assert index not in self.mapIndexesAnnotated, "Found an existing annotation {} at the positions {} when it should be empty for inserting annotation {}, check the code.".format(self.mapIndexesAnnotated[index], index, str(ann))
    ...
    def evalDetectionStrictMatching(self, toponymsPredictedWoExcluded):
        ...
        for topoPred in toponymsPredictedWoExcluded:
            if topoPred.getStartOffset() in self.mapIndexesAnnotated:
                ...
                annotation = self.mapIndexesAnnotated[topoPred.getStartOffset()]#annotation can be an excluded span or toponym for annotation over-spanning
                if type(annotation)==Toponym and topoPred.getStartOffset()==annotation.getStartOffset() and topoPred.getEndOffset()==annotation.getEndOffset():
                    ...
                    if topoPred.getText()!=annotation.getText():
                        ...
                        if topopredWoSpace != annotWoSpace:
                            ...
                            assert topopredWoSpaceWoHyphen==annotWoSpaceWoHyphen, "Found a predicted toponym {} and a reference toponym at the same position with different texts {} and {}, should not be check the data...".format(str(topoPred), str(annotation), topoPred.getText(), annotation.getText())
                ...
                if topoRef.getStartOffset()==topoPred.getStartOffset() and topoRef.getEndOffset()==topoPred.getEndOffset():
                    ...
                    if topoPred.getText()!=topoRef.getText():
                        ...
                        if topopredWoSpace!=toporefWoSpace:
                            ...
                            assert topopredWoSpaceWoHyphen==toporefWoSpaceWoHyphen, "Found a predicted toponym {} and a reference toponym at the same position with different texts {} and {}, should not be check the data...".format(str(topoPred), str(topoRef), topoPred.getText(), topoRef.getText())                                                    
                    found = True
                    break
            if not found:
    ...
    def evalDetectionOverlapMatching(self, toponymsPredictedWoExcluded):
        ...
        for topoRef in self.toponymsReferences:
            found = False
            for topoPred in toponymsPredictedWoExcluded:
                if not ((topoPred.getStartOffset() < topoRef.getStartOffset()) and (topoPred.getEndOffset() < topoRef.getStartOffset()) or
                        (topoRef.getStartOffset() < topoPred.getStartOffset()) and (topoRef.getEndOffset() < topoPred.getStartOffset())):
        ...
        for topoPred in toponymsPredicted:
            if topoPred.getStartOffset() in self.mapIndexesAnnotated:
                ...
                if type(annotation)==ExcludedSpan:
                    ...
                    if (annotation.getStartOffset() <= topoPred.getStartOffset()) and (topoPred.getEndOffset() <= annotation.getEndOffset()):
                        log.debug("Toponym {} has been excluded from evaluation being within an excluded span {}".format(str(topoPred), str(annotation)))
...
class ToponymEvaluator(object):
    ...
    def __init__(self, pathDirPredictions, pathDirReferences):
        ...
        assert len(predictions) == len(references), "The number of prediction files [{}] is not equal to the number of reference files [{}], check the data.".format(len(predictions),len(references))
        for index in range(len(predictions)):
            assert predictions[index]==references[index], "The folders of predictions and references do not contain exactly the same files, I found at index {} in the references folder the file {} where I expected the file {} as in the prediction folder.".format(index, references[index], predictions[index])
        ...
        for index in range(len(predictions)):
            ...
            annotations["predictions"] = parser.parseAnnotations(os.path.join(self.pathDirPredictions,predictions[index]))
...
#             print("{}".format(macroPds))
    ...
    def evalDetection(self, outFile):
        ...
        for doc, annotations in sorted(self.getAnnotations().items()):
            ...
            oracleAnswersStrict[doc].setExcludedToponyms([x for x in annotations['predictions']['toponyms'] if x not in toponymsPredictedWoExcluded])
            oracleAnswersOverlap[doc].setExcludedToponyms([x for x in annotations['predictions']['toponyms'] if x not in toponymsPredictedWoExcluded])
...
#             print(oracleAnswersOverlap[doc])
    ...
    def evalDisambiguation(self, outFile):
        ...
        for doc, annotations in sorted(self.getAnnotations().items()):
            ...
            oracleAnswersStrict[doc] = oracle.evalDetectionStrictMatching(toponymsPredictedWoExcluded)
            oracleAnswersStrict[doc].setExcludedToponyms([x for x in annotations['predictions']['toponyms'] if x not in toponymsPredictedWoExcluded])
            assert len(oracleAnswersStrict[doc].getFNs())==0 and len(oracleAnswersStrict[doc].getFPs())==0, "I found {} FNs and {} FPs which should both be ==0 since we are in desambiguation only, please check the data.".format(len(oracleAnswersStrict[doc].getFNs()), len(oracleAnswersStrict[doc].getFPs())) 
...
if __name__ == '__main__':
    ...
    if evaluation_mode=='Detection':
        ...
        log.fatal("Unknown evaluation parameter received: [{}]. Expecting [Detection, Disambiguation, Resolution]".format(evaluation_mode))
        raise Exception("Unknown evaluation parameter received: [{}]. Expecting [Detection, Disambiguation, Resolution]".format(evaluation_mode))    
